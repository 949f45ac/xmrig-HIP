...
__device__ __forceinline__ uint32_t get_reciprocal_old(const uint32_t* RCP, uint32_t a)
{
...
__device__ __forceinline__ uint32_t fast_sqrt_v2(const uint64_t n1)
{
	...
	const float delta = __int2float_rn(reinterpret_cast<const int32_t*>(&delta0)[1]) * x1;
