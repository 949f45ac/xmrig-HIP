...
{
	dim3 grid, block;
	if (false) { // VARIANT != xmrig::VARIANT_2 && ctx->device_threads > 8) {
...
{
#if ONLY_VEGA
	cryptonight_gpu_hash_shifted<8>(ctx, algo, variant, startNonce);
#else
	if (ctx->device_mpcount > 24) {
		cryptonight_gpu_hash_shifted<8>(ctx, algo, variant, startNonce);
	} else {
		cryptonight_gpu_hash_shifted<6>(ctx, algo, variant, startNonce);
	}
#endif
}
